...
function initializeCardAnimations() {
  ...
  cards.forEach((card, index) => {
    ...
    //
    // *** The conflicting requestAnimationFrame block has been removed ***
    //
  });
}
...
async function fetchProducts(page = 1) {
  ...
  const categoryValue = categoryInput.value; // This will no longer error!
  ...
  try {
    ...
    //
    // *** The conflicting "fallback" setTimeout has been removed ***
    //
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // --- NEW: Check if there's a search parameter in the URL on page load ---
  const urlParams = new URLSearchParams(window.location.search);
  const searchParam = urlParams.get('search');
  if (searchParam && searchInput) {
    // The search input is already populated by PHP, so just trigger the search
    fetchProducts(1);
  }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function openModal(card) {
    ...
    // Get and store stock JSON
    const stockJson = card.getAttribute('data-item-stock-json');
    let stockByPaperType = {};
    try {
      stockByPaperType = stockJson ? JSON.parse(stockJson) : {};
    } catch (e) {
      console.error('Error parsing stock JSON:', e);
    }

    // Reset paper type dropdown and set up change handler
    const modalPaperType = document.getElementById('modal-paper-type');
    const modalPaperTypeInput = document.getElementById('modal-paper-type-input');

    if (modalPaperType) {
      modalPaperType.selectedIndex = 0; // Reset to Vinyl

      // Update stock display based on selected paper type
      const updateStockForPaperType = () => {
        const selectedPaperType = modalPaperType.value;
        const paperTypeStock = stockByPaperType[selectedPaperType] || 0;

        // Update stock display
        if (modalStockValue) {
          if (paperTypeStock > 0) {
            modalStockValue.textContent = paperTypeStock;
            modalStockValue.style.color = ''; // Reset color
            modalStockValue.style.fontWeight = '';
          } else {
            modalStockValue.textContent = "Out of Stock";
            modalStockValue.style.color = 'red';
            modalStockValue.style.fontWeight = 'bold';
          }
        }

        // Update quantity max
        modalQuantityInput.max = paperTypeStock > 0 ? paperTypeStock : 1;
        if (parseInt(modalQuantityInput.value) > paperTypeStock) {
          modalQuantityInput.value = paperTypeStock > 0 ? 1 : 0;
        }

        // Update hidden input for paper type
        if (modalPaperTypeInput) {
          modalPaperTypeInput.value = selectedPaperType;
        }

        // Handle out of stock
        const isPaperTypeOutOfStock = paperTypeStock <= 0;

        if (modalOutOfStockMessage) {
          modalOutOfStockMessage.style.display = isPaperTypeOutOfStock ? 'block' : 'none';
          if (isPaperTypeOutOfStock) {
            modalOutOfStockMessage.textContent = selectedPaperType + ' is currently out of stock.';
          }
        }

        if (modalAddToCartBtn) {
          modalAddToCartBtn.disabled = isPaperTypeOutOfStock;
          modalAddToCartBtn.style.opacity = isPaperTypeOutOfStock ? '0.5' : '1';
          modalAddToCartBtn.style.cursor = isPaperTypeOutOfStock ? 'not-allowed' : 'pointer';
        }

        modalQuantityInput.disabled = isPaperTypeOutOfStock;

        // Update total
        updateModalTotal();
      };

      // Set initial stock for Vinyl
      updateStockForPaperType();

      // Add change event listener
      modalPaperType.addEventListener('change', updateStockForPaperType);
    }
