...
$page_filter = 'newarrivals';
try {
  $total_items_query = $conn->prepare("SELECT COUNT(*) FROM items WHERE page = :page");
  $total_items_query->bindParam(':page', $page_filter);
...
try {
  // Fetch ONLY items for the current page
  $sql = "SELECT * FROM items WHERE page = :page ORDER BY name ASC LIMIT :limit OFFSET :offset"; 
  $stmt = $conn->prepare($sql);
  $stmt->bindParam(':page', $page_filter);
  ...
  // Fetch stocks for these items
  if (!empty($items)) {
      $itemIds = array_column($items, 'id');
      $placeholders = implode(',', array_fill(0, count($itemIds), '?'));
      
      $stockSql = "SELECT item_id, paper_type, stock FROM item_stock WHERE item_id IN ($placeholders)";
      ...
      foreach ($allStocks as $s) {
          $stocksByItem[$s['item_id']][$s['paper_type']] = $s['stock'];
      }
      
      // Merge into items
      foreach ($items as &$item) {
          $item['stocks'] = isset($stocksByItem[$item['id']]) ? $stocksByItem[$item['id']] : [];
      }
      ...
      <div class="left-panel">
        ...
        <div class="filter-game">
          ...
          <div class="game-options">
            ...
            <div class="cntr-rd">
              <input type="radio" id="game-other" name="game-category" class="hidden-xs-up" value="Other">
              <label for="game-other" class="cbx-rd"></label>
              <label for="game-other" class="lbl">Others</label>
            </div>
          </div>
        </div>
      ...
      <div class="main-right-content">
          ...
          ?>
            <div class="card <?php echo $is_out_of_stock ? 'out-of-stock' : ''; ?>" 
                 ...
                 data-item-stock-json="<?php echo htmlspecialchars(json_encode($item['stocks'] ?? [])); ?>">
